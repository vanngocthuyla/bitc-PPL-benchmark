...
rng_key = random.split(random.PRNGKey(2))
print("Running SA: ")
kernel = SA(make_TwoComponentBindingModel)
sa = MCMC(kernel, num_warmup=200, num_samples=5000, num_chains=4)
sa.run(rng_key, q_actual_cal, injection_volumes, CELL_CONCENTR, SYRINGE_CONCENTR)
sa.print_summary()

trace_sa = sa.get_samples(group_by_chain=True)
data = az.convert_to_inference_data(trace_sa)
az.plot_trace(data)

trace_sa = sa.get_samples()
start = {k: trace_sa[k][-1] for k in trace_sa}
print(np.array(start))

print("Updating SA values and running NUTS: ")
kernel = NUTS(make_TwoComponentBindingModel, init_strategy=init_to_value(values=start),)
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "sa_nuts_traceplot.pdf"))
...
az.plot_trace(data_nuts)
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "nuts_traceplot.pdf"))
